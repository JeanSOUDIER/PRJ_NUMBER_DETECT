...
void test_best_fit(void) {
	...
	int j = 0, k = 0;
	while(29 > j) {
        if(!isinf(a.at(0).at(k)) && (a.at(0).at(k) || a.at(1).at(k))) {
            A(j,0) = a.at(0).at(k);
            A(j,1) = a.at(1).at(k);
            A(j,2) = 0;
            //std::cout << static_cast<float>(a.at(0).at(k)) << " " << static_cast<float>(a.at(1).at(k)) << std::endl;
            j++;
        }
        k++;
	}
    ...
    for (int i=0; i < N_tests; i++){
        ...
        k = 0;
        while(29 > j) {
            if(!isinf(b.at(0).at(k)) && (b.at(0).at(k) || b.at(1).at(k))) {
                B(j,0) = b.at(0).at(k);
                B(j,1) = b.at(1).at(k);
                B(j,2) = 0;
                //std::cout << static_cast<float>(b.at(0).at(k)) << " " << static_cast<float>(b.at(1).at(k)) << std::endl;
                j++;
            }
            k++;
        }
