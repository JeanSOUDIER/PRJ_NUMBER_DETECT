...
int main(){
     ...
     const std::vector<Movement> movements_0 = seq_0.getMovements(); //Movements of seq_0 in a std::vector

     const Sequence seq_1 = seqHandler.find('1');//Sequence from 1
     const std::vector<Movement> movement_1 = seq_1.getMovements();//Movements of seq_1 in a std::vector
     ...
     for(unsigned int index = 0 ; index < movements_0.size() ; index++){
         ...
         if(index+1 < movements_0.size()){ //Only for valid instances of nextMovement
             ...
             if(currentMovement.isEquivalent(nextMovement)){/*...*/} //If the two movements are equivalent, do stuff
                 ...
                 std::vector<Movement> segment = Sequence::generateSegment(currentMovement , nextMovement , 1 , 5 , 3 , 10); //Generate a std::vector<Movement>
                 //...
