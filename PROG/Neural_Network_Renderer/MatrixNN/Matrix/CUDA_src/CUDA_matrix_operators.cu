...
std::vector<double> CUDA_mult_MAT(const std::vector<double> &data_1 , const uint64_t data_1_rows, const uint64_t data_1_columns,
...
//    std::cout << "data_2_rows: " << data_2_rows << " data_2_columns: " << data_2_columns << "\n";
    ...
    gpuErrchk(cudaMalloc((void**)&GPU_data_1 , data_1.size()*sizeof(double)));
    gpuErrchk(cudaMemcpy(GPU_data_1, data_1.data(), data_1.size()*sizeof(double), cudaMemcpyHostToDevice));

    double* GPU_data_2 = NULL;
    gpuErrchk(cudaMalloc((void**)&GPU_data_2 ,data_2.size()*sizeof(double)));
    gpuErrchk(cudaMemcpy(GPU_data_2, data_2.data(), data_2.size()*sizeof(double), cudaMemcpyHostToDevice));

    double* GPU_result = NULL;
    gpuErrchk(cudaMalloc((void**)&GPU_result , result.size()*sizeof(double)));
    ...
    const double alpha = 1.f; //Needs to be defined as a variable as it can be either a host or a device pointer (type double* in argument)
    ...
    gpuErrchk(cudaMemcpy(result.data() , GPU_result , result.size() * sizeof(double) , cudaMemcpyDeviceToHost));
