...
std::vector<std::vector<std::string>> readCSV_STR(const std::string &path, const std::string &delimiter){
    ...
    }else{throw std::runtime_error("Utility::readCSV_STR\n Unable to open file.\n");}
...
std::vector<std::vector<float>> readCSV_FLOAT(const std::string &path, const std::string &delimiter){
    ...
    for(auto &vect:str_data){
        ...
        f_vect.reserve(vect.size());

        std::transform(vect.begin(), vect.end() , std::back_inserter(f_vect ), [](std::string str) {return std::stof(str);});

        result.push_back(f_vect);
    }
