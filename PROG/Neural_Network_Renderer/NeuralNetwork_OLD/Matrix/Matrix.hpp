...
/**
    ...
    • remove_row        | Convience function to remove a row at a specified position.
    • remove_line       | Alias for remove_row.
    ...
    • begin_row         | Convinience function that returns 0, to provide syntax as close as the one relative to std::algorithm as possible.
    ...
    • end_row           | Convinience function that returns the number of lines, to provide syntax as close as the one relative to std::algorithm as possible
    ...
    • max               | Returns the maximum of the matrix.
    • min               | Returns the minimum of the matrix.
...
class Matrix{
    ...
    Matrix(const std::vector<T> &data , const uint64_t &rows , const uint64_t&columns){
        ...
        ///rowAt                                                                                    | Returns by reference the row at the specified index.
        std::vector<T>& rowAt(const uint64_t &index){return lineAt(index);}

        ///rowAt                                                                                    | Returns the value of the row at the specified index.
        std::vector<T> rowAt(const uint64_t &index) const{return lineAt(index);}
        ...
        ///replace_row                                                                               | Replaces the line specified in argument by value.
        ...
        ///remove_row                                                                                | Removes a row at the specified index.
        void remove_row(const uint64_t &index){remove(index, Orientation::LINE);}

        ///remove_line                                                                               | Removes a row at the specified index.
        ...
        void print() const{
            ...
            std::cout << "]" << std::fflush; //Buffer flushed to prevent user misusage
        ...
        void print_size() const{
            ...
            std::cout << "[" << rows() << " ; "<< columns() << "]" << std::fflush; //Buffer flushed to prevent user misusage

        }

        ///begin_line | begin_column | begin_line | end_column                                       | Convinience functions that return the beginning or the end of a line / a column. Provided to obtain syntax as close as std::algorithm functions as possible.
        uint64_t begin_row() const{return 0;}
        uint64_t begin_line() const{return 0;}
        uint64_t begin_column() const{return 0;}

        uint64_t end_row() const{return rows();}
        uint64_t end_line() const{return rows();}
        uint64_t end_column() const{return columns();}
        ...
        T max() const{
            ...
            std::vector<T> maxes;
            for(auto & line : _data){
                maxes.push_back(*std::max_element(line.begin() , line.end()));
            }
        ...
        T min() const{
            ...
            std::vector<T> mins;
            for(auto & line : _data){
                mins.push_back(*std::min_element(line.begin() , line.end()));
            }
