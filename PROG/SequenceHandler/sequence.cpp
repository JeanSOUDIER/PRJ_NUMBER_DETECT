...
Sequence::Sequence(QVector<Movement> mov){movements = mov;}
Sequence::Sequence(std::vector<Movement> mov){movements = QVector<Movement>::fromStdVector(mov);}


/**************************************************************/

void Sequence::addMovement(Movement mov){movements.append(Movement(mov));}
void Sequence::removeMovement(const unsigned index){movements.remove(index);}
void Sequence::replaceMovement(const unsigned index, Movement mov){movements.replace(index, Movement(mov));}
void Sequence::clear(){
...
QVector<Movement> Sequence::generateSegment(const Movement &begin , const Movement &end , const uint64_t step_x, const uint64_t step_y, const uint64_t step_z , const uint64_t delay_ms){

    if(begin.positionMatches(end)){return QVector<Movement>{begin , end};}

    Movement currentMovement = begin;
    QVector<Movement> output = {Movement(currentMovement)};
   ...
   /* qDebug() << "x_is_inveted: " << x_is_inverted;
    ...
    while(!ended){
        ...
        if(!ended){output.append(Movement(currentMovement));};
    ...
    if(!output.at(output.size()-1).positionMatches(end)){output.append(end);}
...
std::vector<Movement> Sequence::generateSegment_STD(const Movement &begin , const Movement &end , const uint64_t step_x, const uint64_t step_y, const uint64_t step_z , const uint64_t delay_ms) {
    return Sequence::generateSegment(begin , end , step_x , step_y , step_z , delay_ms).toStdVector();
}



QVector<Movement> Sequence::generateSegment(const Movement &begin , const Movement &end , const uint64_t step , const uint64_t delay_ms){
    return Sequence::generateSegment(begin , end , step , step , step , delay_ms);
}

std::vector<Movement> Sequence::generateSegment_STD(const Movement &begin , const Movement &end , const uint64_t step , const uint64_t delay_ms){
    return Sequence::generateSegment_STD(begin , end , step , step , step , delay_ms);
}



QVector<Movement> Sequence::generateSegment(const Movement &begin , const Movement &end , const uint64_t step){
    return Sequence::generateSegment(begin , end , step , step , step , 100);

}
std::vector<Movement> Sequence::generateSegment_STD(const Movement &begin , const Movement &end , const uint64_t step){
    return Sequence::generateSegment_STD(begin , end , step , step , step , 100);
}




QVector<Movement> Sequence::generateSegment(const Movement &begin , const Movement &end){
    return Sequence::generateSegment(begin , end , 1 , 1 , 1 , 100);
}
std::vector<Movement> Sequence::generateSegment_STD(const Movement &begin , const Movement &end){
    return Sequence::generateSegment_STD(begin , end , 1 , 1 , 1 , 100);
}
/**************************************************************/
